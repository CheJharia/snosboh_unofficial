...
library(tidyverse)
client_name <- 'HAU_UTSYDCRM' # University of Technology Sydney
...
Dsynergygdw.madeOfferIntCommunication_data <- dplyr::tbl_df(RODBC::sqlQuery(dbhandle, query_IntMadeOfferComm))
...
Dsynergygdw.enquiryIntCommunication_data <- dplyr::tbl_df(RODBC::sqlQuery(dbhandle, query_IntEnquiryComm))
...
Dsynergygdw.madeOfferDomCommunication_data <- dplyr::tbl_df(RODBC::sqlQuery(dbhandle, query_DomMadeOfferComm))
...
Dsynergygdw.enquiryDomCommunication_data <- dplyr::tbl_df(RODBC::sqlQuery(dbhandle, query_DomEnquiryComm))
...
stats_class[is.na(stats_class)] <- 0                
FDomEnq <- cbind(FDomEnq, stats_class)
...
stats_class[is.na(stats_class)] <- 0                
FDomMO <- cbind(FDomMO, stats_class)
