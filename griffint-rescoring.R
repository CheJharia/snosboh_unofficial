...
library(magrittr)
library(pca3d)
traindata <- tbl_df(read.csv("GRIFFINT-raw-scores.csv", header = T, stringsAsFactors = F))
...
fviz_contrib(CL_res.pca, choice = "var", axes = 1:2, top = 3)
...
#################################################################################
#################################################################################
fviz_pca_biplot(CL_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 1), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()  + labs(title = "Cost of Living - Respondents and Questions Plot")
...
names(traindata)[12:20]
EM <- c(12:20)
EM_pca_data <- tbl_df(traindata[,c(1,EM)])
library("FactoMineR")
EM_res.pca <- PCA(EM_pca_data[,2:10], graph = TRUE)
print(EM_res.pca)
...
cont_table[,3] + cont_table[,2] + cont_table[,1]
...
#################################################################################
#################################################################################
fviz_pca_biplot(EM_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 1), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()   + labs(title = "Employment - Respondents and Questions Plot")

fviz_pca_biplot(EM_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=15, height = 15), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5,habillage = traindata$`Profile New` ) +
  theme_minimal() 


pca <- prcomp( EM_pca_data[,2:10], scale.= TRUE )
...
############################################################ Lifestyle PCA
names(traindata)[23:29]
LE <- c(23:29)
LE_pca_data <- tbl_df(traindata[,c(1,LE)])
library("FactoMineR")
LE_res.pca <- PCA(LE_pca_data[,2:8], graph = TRUE)
print(LE_res.pca)
...
cont_table[,3] + cont_table[,2] + cont_table[,1]
fviz_contrib(LE_res.pca, choice = "var", axes = 1:3, top = 8)
...
#################################################################################
#################################################################################
fviz_pca_biplot(LE_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 1), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()   + labs(title = "Lifestyle - Respondents and Questions Plot")
...
pca <- prcomp( LE_pca_data[,2:8], scale.= TRUE )
pca3d(pca, group=traindata$`Profile New`,col = traindata$warna)
makeMoviePCA()
#################################################################################
#################################################################################
...
names(traindata)[32:40]
TQ <- c(32:40)
TQ_pca_data <- tbl_df(traindata[,c(1,TQ)])
library("FactoMineR")
TQ_res.pca  <- PCA(TQ_pca_data[,2:10], graph = TRUE)
print(TQ_res.pca)
...
#################################################################################
#################################################################################
fviz_pca_biplot(TQ_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 1), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()   + labs(title = "Teaching Quality - Respondents and Questions Plot")
...
pca <- prcomp( TQ_pca_data[,2:10], scale.= TRUE )
pca3d(pca, group=traindata$`Profile New`,col = traindata$warna)
makeMoviePCA()
#################################################################################
#################################################################################
...
library("factoextra")
fviz_screeplot(UB_res.pca, ncp=10, addlabels = T) + theme_minimal() + labs(title = "Principal Component Analysis - University Brand",
                                                                           x = "Principal Components", y = "% of variances")
?fviz_screeplot
# Coordinates of variables
...
#################################################################################
#################################################################################
fviz_pca_biplot(UB_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 1), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()   + labs(title = "University Brand - Respondents and Questions Plot")
...
pca_fun_only <- tbl_df(read.csv(file = 'uws-pca-3d-fun.csv',header = T, check.names = F))
...
library("FactoMineR")
uws_res.pca <- PCA(pca_fun_only[1:5], graph = TRUE)

fviz_pca_biplot(uws_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=1, height = 10), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5) +
  theme_minimal()   + labs(title = "University Brand - Principal Component Analysis")

fviz_pca_biplot(uws_res.pca, label ="var", col.ind="cos2", jitter = list(what = c("b"), width=10, height = 15), repel = T, col.var = 'black',addEllipses = TRUE, alpha.ind = 0.5, labelsize = 4.5,habillage =  traindata$`Profile New` ) +
  theme_minimal() 
