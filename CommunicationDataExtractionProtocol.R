...
setwd("C:/Users/afiq.johari/Documents/Data Scientist/Incoming_Data/!Communication/New")
...
length(Enq_files)
# Preliminary verification (ClientName is used for filtering)
for (fe in Enq_files) {
...
for (gfile in Enq_files) {
  ...
  (dirpath <- paste("C:/Users/afiq.johari/Documents/Data Scientist/Incoming_Data/!Communication",'/!Files/',fname, sep = ""))
  dir.create(dirpath)
  ...
  ##################################################
  colnames(f)[1] <- 'ClientName'
  colnames(f)
  
  ##################################################
  # Get all distinct clients in the file
  ##################################################
  clients <- as.data.frame(fn$sqldf("select distinct ClientName from f"))
  # get rid of the annoying factor levels
  clients <- levels(factor(clients$ClientName))
  ...
  for (client in clients) {
    ...
    csvfilename <- paste(subdirpath,'/','COM_',client,'_',fdate,'.csv', sep = "")
    # output unique records to csv file
    write.csv(unique(fn$sqldf("select * from f where ClientName = '$client'")) ,file = csvfilename, row.names = F)
  }
