...
setwd("C:/Users/afiq.johari/Documents/Data Scientist/Incoming_Data/!Retention/New")
...
for (gfile in Enq_files) {
  ...
  (dirpath <- paste("C:/Users/afiq.johari/Documents/Data Scientist/Incoming_Data/!Retention",'/!Files/',fname, sep = ""))
  dir.create(dirpath)
  ...
  ##################################################
  colnames(f)
  colnames(f)[1] <- 'client_id'
  colnames(f)
  
  ##################################################
  # Get all distinct clients in the file
  ##################################################
  clients <- as.data.frame(fn$sqldf("select distinct client_id from f"))
  # get rid of the annoying factor levels
  clients <- levels(factor(clients$client_id))
  ...
  for (client in clients) {
    ...
    csvfilename <- paste(subdirpath,'/','ENQ_',client,'_',fdate,'.csv', sep = "")
    # output unique records to csv file
    write.csv(unique(fn$sqldf("select * from f where client_id = '$client'")) ,file = csvfilename, row.names = F)
  }
