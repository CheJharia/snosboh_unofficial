...
res.hc4 <- eclust(data, "hclust", k = 4,
                  method = "complete", graph = FALSE) 
res.hc5 <- eclust(data, "hclust", k = 5,
                 method = "complete", graph = FALSE) 
res.hc6 <- eclust(data, "hclust", k = 6,
                 method = "complete", graph = FALSE) 
head(res.hc5$cluster, 15)
?eclust
# Dendrogram
fviz_dend(res.hc4, rect = TRUE, show_labels = FALSE) 
fviz_dend(res.hc5, rect = TRUE, show_labels = FALSE) 
