...
Ddummyranked %>% dplyr::select(starts_with("CP")) %>% nscumcomp(nneg = TRUE) %>% .$rotation %>% rowSums(.)

CP_nnegPCA_score <- tbl_df(as.matrix(Ddummyranked %>% dplyr::select(starts_with("CP"))) %*% as.matrix(Ddummyranked %>% dplyr::select(starts_with("CP")) %>% nscumcomp(nneg = TRUE) %>% .$rotation) %>% rowSums(.))
