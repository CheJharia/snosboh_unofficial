...
year <- dplyr::tbl_df(c(2012,2013,2014))
colnames(year) <- "year"
url <- dplyr::tbl_df(c("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/DVS/mortality/mort2012us.zip"
...
colnames(url) <- "url"
POP <- bind_cols(year,url)
# Now run the function for each year you want:
for (i in 1:nrow(POP)) {
  year <- as.integer(POP[i,1])
  url <- as.character(POP[i,2])
  CDC_parser(year, url)
}
