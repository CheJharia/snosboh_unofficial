...
for (client in Lclient) {
  ...
  Qcomm <- paste("SELECT *
                 ...
                 WHERE client_id = '", client,"' AND class = 'Phone' " , sep = '')
  ...
  if (nrow(DComm) > 0) {
    ...
    print(ggplot(Dcomm.raw, aes(x = Week, y = Day, fill = daily_comm)) +
            ...
            theme_tufte() + coord_equal() + ggtitle(paste("Heatmap of Weekly Number of Phone Communication : ", client,sep = "")))
    # save plot
    #dev.copy(png, paste(getwd(), '/', client, '_heatmap_overall', '.png' ,sep = ''))
    dev.copy(png, paste(getwd(), '/', client, '/',client,'_heatmap_phone_overall', '.png' ,sep = ''), width = 1280, height = 1024, res = 100)
    dev.off()
    ...
    for (y in years) {
      ...
      print(ggplot(Dy, aes(x = Week, y = Day, fill = daily_comm)) +
              ...
              scale_x_continuous(expand = c(0, 0), breaks = seq(1, 52, length = 12),
                                 labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")) +
              theme_tufte() + coord_equal() + ggtitle(paste("Heatmap of Weekly Number of Phone Communication : ", client," ", y,sep = "")))
      dev.copy(png, paste(getwd(), '/', client, '/',client, '_heatmap_phone', y , '.png' ,sep = ''), width = 1280, height = 1024, res = 100)
      dev.off()
    }
