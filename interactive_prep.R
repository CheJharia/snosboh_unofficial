...
guns_for_interactive <- all_guns %>%
  mutate(age_group = cut(age, breaks = c(-1,14,34,64,107), labels = c("0-14", "15-34", "35-64", "65+")),
         ...
         age_group = factor(age_group, labels = c("0-14", "15-34", "35-64", "65+", "Unknown"))) %>% select(year, intent, sex, age_group, race)
...
ACS <- read_csv("usa_00002.csv.gz") # Or change the file name as needed.
