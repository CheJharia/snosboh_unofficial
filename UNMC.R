...
client <- 'HAU_UNONTNGM'
DCommStudUser_client <- DCommStudUser %>%
  filter(client_id == client)

# USER

DCommStudUser_client_user_all_comm <- DCommStudUser_client %>%
  group_by(username, Year, Month, Day, Hour) %>%
  summarise(year_month_hour_all_comm = n())

unique(DCommStudUser$id)

ymin <- 2010
ymax <- 2016
df <- DCommStudUser_client_user_all_comm %>% filter( Year >= ymin & Year <= ymax) %>%
  filter(Hour >= 8 & Hour <= 20, Day %in% c("Mon", "Tue", "Wed", "Thu","Fri"), username == 'aida.annuar')
...
All communication University of Nottingham Malaysia")
...
library(dplyr)
DCommStudUser %>% group_by(country_of_residence) %>% summarise(n = n()) %>% arrange(desc(n))
