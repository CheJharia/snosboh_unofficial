...
if (small_areas){
  data_raw <- read.csv("AllThemesTablesSA.csv")
  ireland_map <- readShapePoly('Census2011_Small_Areas_generalised20m.shp')
  ...
  data_raw <- read.csv("AllThemesTablesED.csv")
  names(data_raw)[1] <- "GEOGID"
  ireland_map <- readShapePoly('Census2011_Electoral_Divisions_generalised20m.shp')
